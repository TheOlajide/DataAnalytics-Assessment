...
WITH table1 AS (
SELECT
 users.id AS customer, EXTRACT(MONTH FROM transaction_date) AS month, COUNT(*) AS total_txn
FROM
...
GROUP BY users.id,EXTRACT(MONTH FROM transaction_date)
),
table2 AS (
SELECT
...
SELECT
 avg_transactions_per_month, COUNT(*) AS customer_count , frequency_category
FROM
 table3
GROUP BY
